...
public class GoogleMa extends FragmentActivity implements View.OnClickListener, OnMapReadyCallback {

    private static final String TAG = GoogleMa.class.getName();
    ...
    private GPSData.GPSLocation yow = new GPSData.GPSLocation(ge);

    public GPSData.GPSLocation getYow() {
    ...
    private double droneLocationLat = locatie.getLatitude(), droneLocationLng = 181;
        ...
        UserAccountManager.getInstance().logIntoDJIUserAccount(this,
                new CommonCallbacks.CompletionCallbackWith<UserAccountState>() {
                    ...
                    public void onSuccess(final UserAccountState userAccountState) {
                        ...
                        GoogleMa.this.runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
    ...
    private void updateTitleBar() {
        ...
        boolean ret = false;
        BaseProduct product = GEOtest.getProductInstance();
        if (product != null) {
            if (product.isConnected()) {
                //The product is connected
                GoogleMa.this.runOnUiThread(new Runnable() {
                    public void run() {
                        mConnectStatusTextView.setText(GEOtest.getProductInstance().getModel() + " Connected");
                    }
                });
                ret = true;
            } else {
    ...
    public void showToast(final String msg) {
        GoogleMa.this.runOnUiThread(new Runnable() {
            public void run() {
                Toast.makeText(GoogleMa.this, msg, Toast.LENGTH_SHORT).show();
            }
        });
    }
            ...
            case R.id.geo_get_custom_unlock_zones:
                DJISDKManager.getInstance().getFlyZoneManager().getCustomUnlockZonesFromAircraft(new CommonCallbacks.CompletionCallbackWith<List<CustomUnlockZone>>() {
                    ...
                    public void onSuccess(final List<CustomUnlockZone> customUnlockZones) {
                        ...
                        GoogleMa.this.customUnlockZones = (ArrayList<CustomUnlockZone>) customUnlockZones;
    ...
    public void onMapReady(GoogleMap googleMap) {
        LatLng KortrijkVives = new LatLng(50.805638, 3.282862);

        mMap = googleMap;
        if (latLng != null) {
            mMap.moveCamera(CameraUpdateFactory.newLatLng(latLng));
        } else {
            mMap.moveCamera(CameraUpdateFactory.newLatLng(KortrijkVives));
        }
